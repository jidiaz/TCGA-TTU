...
comparisons <- read.csv(file="/Users/juansola/Desktop/TCGA-TTU/TCGA2/240503_TCGA_CPTAC-3_Output/240503_TCGA_CPTAC-3_DEA/240503_TCGA_CPTAC-3_DEA_Input/directory_mappingcopy.csv",header = FALSE)

for (x in 1:nrow(comparisons)){

name=comparisons[x,1]
name=strsplit(name, split = "/")
name=strsplit(name[[1]][length(name[[1]])], split = "_DES.csv")



#change X with the complete path to your raw count file
raw_count_file=comparisons[x,2]

#change X with the complete path to your condition file
condition_file=comparisons[x,1]
...
Humannet_library <- read.csv(file="/Users/juansola/Documents/TC3R/research/092023_Sima_experiment22/Report_RNAseq_SIMA_experiment/240422_Comparison_TCGA_pacreas/240422_Comparison_TCGA_pacreas_inputs/240510_network/HumanNet-XC.tsv",sep = '\t', header=TRUE)  # para el jason
...
# heavy_edge_node_names <- ends(g, E(g)[bridges(g)], names = TRUE)

# for (x in 1:nrow(heavy_edge_node_names)){
#     initial = heavy_edge_node_names[x,1]
#     end_node = heavy_edge_node_names[x,2]
#     cc <- paste(initial,end_node,sep="-")
#     cat(cc, file = file_net_sta)
#     cat("\n", file = file_net_sta)
# }
